...
public class IsilBuilder
{
    ...
    {
        // Sorted list of every address that actually produced ISIL, so we can resolve a branch that
        // lands on an address which emitted nothing (ADRP, DMB, skipped stack-check thunks, etc.) to the
        // next real instruction. That is the correct fall-through semantics: branching "into" a no-op
        // simply continues at the following instruction. Previously any such branch was Invalidated,
        // which destroyed control flow on ARM64 (where ADRP/barriers are extremely common) and was the
        // root cause of the "Jump target not found in method" failures.
        ulong[]? sortedAddresses = null;

        foreach (var tuple in _jumpsToFix)
        {
            var resolved = ResolveJumpTarget(tuple.Item2, ref sortedAddresses);
            ...
            if (resolved.Equals(tuple.Item1))
                tuple.Item1.Invalidate("Invalid jump target for instruction: Instruction can't jump to itself");
            else
                tuple.Item1.Operands = [InstructionSetIndependentOperand.MakeInstruction(resolved)];
        }
    }

    private InstructionSetIndependentInstruction? ResolveJumpTarget(ulong target, ref ulong[]? sortedAddresses)
    {
        if (InstructionAddressMap.TryGetValue(target, out var exact))
            return exact.First();

        sortedAddresses ??= InstructionAddressMap.Keys.OrderBy(a => a).ToArray();

        // Find the first emitted instruction at an address >= the requested target (the next real
        // instruction after a no-op the branch landed on). Binary search for the lower bound.
        var lo = 0;
        var hi = sortedAddresses.Length;
        while (lo < hi)
        {
            var mid = (lo + hi) / 2;
            if (sortedAddresses[mid] < target)
                lo = mid + 1;
            else
                hi = mid;
        }

        if (lo < sortedAddresses.Length)
            return InstructionAddressMap[sortedAddresses[lo]].First();
